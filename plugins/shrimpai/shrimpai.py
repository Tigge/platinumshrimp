...
class shrimpai(plugin.Plugin):
    def __init__(self):
        plugin.Plugin.__init__(self, "shrimpai")
    ...
    def started(self, settings):
        ...
        self.update_count = 0
        logging.info(
            "shrimpai: Trigger: %s, channel: %s, model: %s, max_tokens: %i, temp: %i",
            self.trigger,
            self.channel,
            self.model,
            self.max_tokens,
    ...
    def update(self):
        ...
        if self.update_count >= self.history_timeout:
            logging.info("shrimpai: Resetting history: []")
            self.history = []
            self.update_count = 0

    def reset_history(self, server, channel):
        logging.info("shrimpai: Manual history reset")
        self.history = []
